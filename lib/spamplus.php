...
function SpamCheckBAN($ip)
{
    ...
    if (!$use_spam_check['page_view'])
        return false;
    $obj = new SPAMBAN();
    if ($obj->BlackCheck($ip, $log_ua))
        return true;
    return false;
}
...
class SPAMCHECK
{
    ...
    function setBlockList($x)
    {
        $this->BlockList = $x[0];
    }

    function setBlackList($x)
    {
        $this->BlackList = $x;
    }

    function setWhiteList($x)
    {
        $this->WhiteList = $x[0];
    }

    function setMyNetList($x)
    {
        $this->MyNetList = $x[0];
    }
...
class DNSBL extends SPAMCHECK
{
    ...
    function isListed()
    {
        ...
        if (isset($this->WhiteList[$host]) && $this->WhiteList[$host][0])
            return false;
