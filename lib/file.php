...
function page_write($page, $postdata, $notimestamp = false)
{
    ...
    if ($autoalias && $page == $aliaspage) {
        ...
        if (empty($aliases)) {
            ...
            autolink_pattern_write(
                CACHE_DIR . PKWK_AUTOALIAS_REGEX_CACHE,
    ...
    if ($autoglossary && $page == $glossarypage) {
        ...
        if (empty($words)) {
            ...
            // Create or Update
            autolink_pattern_write(
                CACHE_DIR . PKWK_GLOSSARY_REGEX_CACHE,
...
function file_write($dir, $page, $str, $notimestamp = false)
{
    ...
    if ($dir == DATA_DIR && $str === '') {
        // Page deletion
        if (!$file_exists)
            return; // Ignore null posting for DATA_DIR
...
function add_recent($page, $recentpage, $subject = '', $limit = 0)
{
    ...
    if (auth::check_role('readonly') || $limit == 0 || $page == '' || $recentpage == '' || check_non_list($page))
        return;
    ...
    array_unshift(
        $lines,
        '-' . format_date(UTIME) . ' - ' . $_page . htmlspecialchars($subject) . "\n",
        ENT_QUOTES,
        'UTF-8',
    );
...
function lastmodified_add($update = '', $remove = '')
{
    ...
    pkwk_touch_file($file);
    ($fp = fopen($file, 'r+')) or die_message('Cannot open ' . htmlspecialchars($whatsnew, ENT_QUOTES, 'UTF-8'));
    set_file_buffer($fp, 0);
    @flock($fp, LOCK_EX);
    $last = ignore_user_abort(1);
    ftruncate($fp, 0);
    ...
    foreach ($recent_pages as $page => $time)
        fputs(
            $fp,
            '-' .
            htmlspecialchars(format_date($time), ENT_QUOTES, 'UTF-8') .
                ' - ' .
                '[[' .
                htmlspecialchars($page, ENT_QUOTES, 'UTF-8') .
                ']]' .
                ...
                case 'none':
                    ...
                    foreach ($readings as $page => $reading) {
                        ...
                        foreach ($patterns as $no => $pattern)
                            $readings[$page] = mb_convert_kana(
                                mb_ereg_replace($pattern, $replacements[$no], $readings[$page]),
                                'aKCV',
                            );
                    }
                    break;
...
function pkwk_chown($filename, $preserve_time = true)
{
    ...
    ($flock = fopen($lockfile, 'a')) or
        die('pkwk_chown(): fopen() failed for: CACHEDIR/' . basename(htmlspecialchars($lockfile, ENT_QUOTES, 'UTF-8')));
    ...
    if ($stat[4] === $php_uid) {
        ...
        ($ffile = fopen($filename, 'r')) or
            die('pkwk_chown(): fopen() failed for: ' . basename(htmlspecialchars($filename, ENT_QUOTES, 'UTF-8')));
