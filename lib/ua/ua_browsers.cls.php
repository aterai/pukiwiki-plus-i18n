...
class ua_browsers
{
    ...
    var $browsers_id = array(
        ...
        'real',
        'winamp',
        // これは、winampmpeg および winamp3httprdr に左右する
        ...
        // 一番最後に定義すべきもの
        'mozilla',
        // 大多数のブラウザは、mozila 文字列を含んでいる
        'libwww',
        // libwww を利用するブラウザは、ブラウザ識別子と libwww の両方を含むため
        'lwp',
    );
    ...
    function set_browsers_icon($ua)
    {
        ...
        // IE ?
        if (preg_match($this->regvermsie, $ua, $regs) && !preg_match($this->regnotie, $ua, $tmp)) {
        ...
        // Netscape 3.x, 4.x ... ?
        if (preg_match($this->regvermozilla, $ua, $regs) && !preg_match($this->regnotnetscape, $ua, $tmp)) {
            return $this->browsers_icon['netscape'];
        }
