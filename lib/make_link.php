...
class InlineConverter
{
    ...
    function replace($arr)
    {
        ...
        $this->result[] =
            $obj !== null && $obj->set($arr, $this->page) !== false
                ? $obj->toString()
                : make_line_rules(htmlspecialchars($arr[0], ENT_NOQUOTES, 'UTF-8'));
...
class Link
{
    ...
    function setParam($page, $name, $body, $type = '', $alias = '')
    {
        ...
        if (!PKWK_DISABLE_INLINE_IMAGE_FROM_URI && is_url($alias) && preg_match('/\.(gif|png|jpe?g)$/i', $alias)) {
            $alias = '<img src="' . htmlspecialchars($alias, ENT_QUOTES, 'UTF-8') . '" alt="' . $name . '" />';
        } else if ($alias != '') {
            if ($converter === null)
...
class Link_plugin extends Link
{
    ...
    function toString()
    {
        ...
        if ($str !== false) {
            ...
            $body = ($body == '' ? '' : ('{' . $body . '}')) . ';';
            return make_line_rules(htmlspecialchars('&' . $this->plain, ENT_QUOTES, 'UTF-8') . $body);
        }
    }
}
...
class Link_url extends Link
{
    ...
    function toString()
    {
        ...
        return open_uri_in_new_window(
            '<a href="' . $target . '"' . $rel . '>' . $this->alias . '</a>',
            get_class($this),
        );
    }
}
...
class Link_url_interwiki extends Link
{
    ...
    function set($arr, $page)
    {
        ...
        return parent::setParam($page, htmlspecialchars($name, ENT_QUOTES, 'UTF-8'), '', 'url', $alias);
    }
...
class Link_interwikiname extends Link
{
    ...
    function set($arr, $page)
    {
        ...
        return parent::setParam(
            $page,
            htmlspecialchars($name . ':' . $this->param, ENT_QUOTES, 'UTF-8'),
            '',
            'InterWikiName',
...
function make_tooltips($term, $glossary_page = '')
{
    ...
    $s_term = str_replace("'", "\\'", htmlspecialchars($term, ENT_QUOTES, 'UTF-8'));
    $s_glossary = htmlspecialchars($glossary, ENT_QUOTES, 'UTF-8');
    ...
    if (is_page($page)) {
        ...
        return (
            '<span class="tooltip"' .
            ' onmouseover="' .
            "javascript:this.style.backgroundColor='#ffe4e1';showGlossaryPopup('" .
            $script .
            '?plugin=tooltip&amp;q=' .
            $s_term .
            "',event,0.2);" .
...
function make_pagelink($page, $alias = '', $anchor = '', $refer = '', $isautolink = false)
{
    ...
    $s_page = htmlspecialchars(strip_bracket($page), ENT_QUOTES, 'UTF-8');
    if (!is_page($page)) {
