...
if (!defined('SEGMENTS_END')) {
    ...
    define('SEGMENTS_END', ~0);

    // Marker for the end of the segments[] array.
}

if (!defined('MO_LITTLE_ENDIAN')) {
    define('MO_LITTLE_ENDIAN', 0);

    // Intel system
}
if (!defined('MO_BIG_ENDIAN')) {
    define('MO_BIG_ENDIAN', 1);
...
function bindtextdomain($domain, $directory)
{
    ...
    if (!isset($po_category))
        $po_category = 'LC_MESSAGES';

    if (isset($po_filename[$domain][$po_category]))
        return;

    $locale = defined('PO_LANG') ? PO_LANG : 'ja_JP';
    ...
    if (file_exists($po_filename[$domain][$po_category]))
...
function textdomain($text_domain)
{
    ...
    if (isset($po_msg[$po_domain][$po_category]))
        return $text_domain;
    // A check of finishing [ execution of bindtextdomain ]
    if (!isset($po_filename[$po_domain][$po_category]))
        return $text_domain;
...
function _($message)
{
    ...
    if (!isset($po_msg[$po_domain][$po_category][$msg]))
        return $message;
    ...
    if ($po_charset[$po_domain][$po_category] != $po_codeset[$po_domain]) {
        ...
        return mb_convert_encoding(
            str_replace('\"', '"', $po_msg[$po_domain][$po_category][$msg]),
            $po_codeset[$po_domain],
            $charset,
        );
    }
...
class read_mo
{
    ...
    function destruct()
    {
        unset($this->filename, $this->data, $this->size, $this->fd, $this->endian, $this->charset);
    }

    function read_mo_file($filename)
    {
        if (!$this->read_binary_mo_file($filename))
            return array();
        $this->get_endian();
    ...
    function get_sysdep_string($offset, $header)
    {
        ...
        for ($i = 4;; $i += 8) {
            ...
            // File is truncated.
            if (($ss_offset + $ss_length) > $this->size)
                return '';
    ...
    function offsetof($field)
    {
        ...
        if (empty($mo_file_header))
            $mo_file_header = array(
                'magic' => $base * 0,
                // The magic number.
                'revision' => $base * 1,
                ...
                'orig_tab_offset' => $base * 3,
                // Offset of table with start offsets of original strings.
                'trans_tab_offset' => $base * 4,
                // Offset of table with start offsets of translated strings.
                'hash_tab_size' => $base * 5,
                // Size of hash table.
                'hash_tab_offset' => $base * 6,
                // Offset of first hash table entry.

                // The following are only used in .mo files with minor revision >= 1.
                'n_sysdep_segments' => $base * 8,
                // The number of system dependent segments.
                'sysdep_segments_offset' => $base * 10,
                // ffset of table describing system dependent segments.
                'n_sysdep_strings' => $base * 12,
                // The number of system dependent strings pairs.
                'orig_sysdep_tab_offset' => $base * 14,
                // Offset of table with start offsets of original sysdep strings.
                'trans_sysdep_tab_offset' => $base * 16,
    ...
    function conversion_special_character($x)
    {
        ...
        foreach ($table as $org => $new) {
            $x = str_replace($org, $new, $x);
        }
        return $x;
    }
