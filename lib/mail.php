...
function pkwk_mail_notify($subject, $message, $summary = array(), $summary_position = false)
{
    ...
    if ($subject == '' || $message == '' && empty($summary))
        return false;

    // Subject:
    if (isset($summary['PAGE']))
        $subject = str_replace('$page', $summary['PAGE'], $subject);

    // Summary
    if (isset($summary['REMOTE_ADDR']))
        $summary['REMOTE_ADDR'] = &$_SERVER['REMOTE_ADDR'];
