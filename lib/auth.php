...
function pkwk_login($pass = '')
{
    ...
    if (!auth::check_role('readonly') && isset($adminpass) && pkwk_hash_compute($pass, $adminpass) === $adminpass) {
        return true;
    } else {
...
function pkwk_hash_compute($phrase = '', $scheme = '{php_md5}', $prefix = true, $canonical = false)
{
    ...
    switch (strtolower($scheme)) {
        ...
        case '{x-php-crypt}':
            $hash =
                ($prefix ? ($canonical ? '{x-php-crypt}' : $scheme) : '') .
        ...
        case '{crypt}':
            $hash =
                ($prefix ? ($canonical ? '{CRYPT}' : $scheme) : '') .
                ($salt != '' ? crypt($phrase, $salt) : crypt($phrase));
            break;

        // LDAP MD5
        case '{md5}':
            $hash = ($prefix ? ($canonical ? '{MD5}' : $scheme) : '') . base64_encode(hex2bin(md5($phrase)));
            break;
        ...
        case '{smd5}':
            ...
            $hash =
                ($prefix ? ($canonical ? '{SMD5}' : $scheme) : '') .
                base64_encode(hex2bin(md5($phrase . $salt)) . $salt);
            break;

        // LDAP SHA
        case '{sha}':
            $hash = ($prefix ? ($canonical ? '{SHA}' : $scheme) : '') . base64_encode(hex2bin(sha1($phrase)));
            break;
...
function edit_auth($page, $auth_flag = true, $exit_flag = true)
{
    ...
    if ($auth_flag && !$auth_func_name($page, $auth_flag, $exit_flag, $edit_auth_pages, $_title['cannotedit']))
        return false;
    if (auth::is_page_readable($page, '', '') && auth::is_page_editable($page, '', ''))
        return true;
...
function basic_auth($page, $auth_flag, $exit_flag, $auth_pages, $title_cannot)
{
    ...
    if (
        !isset($_SERVER['PHP_AUTH_USER']) ||
            ...
            pkwk_hash_compute($_SERVER['PHP_AUTH_PW'], $auth_users[$_SERVER['PHP_AUTH_USER']][0]) !==
        ...
        if ($exit_flag) {
            $body =
                $title = str_replace('$1', htmlspecialchars(strip_bracket($page), ENT_QUOTES, 'UTF-8'), $title_cannot);
...
function digest_auth($page, $auth_flag, $exit_flag, $auth_pages, $title_cannot)
{
    ...
    if ($exit_flag) {
        $body = $title = str_replace('$1', htmlspecialchars(strip_bracket($page), ENT_QUOTES, 'UTF-8'), $title_cannot);
        $page = str_replace('$1', make_search($page), $title_cannot);
        catbody($title, $page, $body);
        exit();
    }
