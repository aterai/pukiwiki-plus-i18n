...
const PKWK_HTTP_REQUEST_TIMEOUT = 8;

const PKWK_HTTP_CONNECT_TIMEOUT = 2;

const PKWK_HTTP_VERSION = '1.1';

const PKWK_HTTP_CLIENT = 'PukiWiki/1.4';
...
function in_the_net($networks = array(), $host = '')
{
    ...
    foreach ($networks as $network) {
        if (preg_match(PKWK_CIDR_NETWORK_REGEX, $network, $matches) && is_long($l_ip) && long2ip($l_ip) == $ip) {
