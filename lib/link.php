...
function links_update($page)
{
    ...
    if ($time) {
        // ページが存在している
        if (!empty($rel_new)) {
            ($fp = fopen($rel_file, 'w')) or
                die_message('cannot write ' . htmlspecialchars($rel_file, ENT_QUOTES, 'UTF-8'));
...
function links_init()
{
    ...
    foreach (get_existpages() as $page) {
        ...
        if (!empty($rel)) {
            ($fp = fopen(CACHE_DIR . encode($page) . '.rel', 'w')) or
                die_message('cannot write ' .
                    htmlspecialchars(CACHE_DIR . encode($page) . '.rel', ENT_QUOTES, 'UTF-8'));
            fputs($fp, join("\t", $rel));
            fclose($fp);
        }
    }
    ...
    foreach ($ref as $page => $arr) {
        ($fp = fopen(CACHE_DIR . encode($page) . '.ref', 'w')) or
            die_message('cannot write ' . htmlspecialchars(CACHE_DIR . encode($page) . '.ref', ENT_QUOTES, 'UTF-8'));
...
function links_add($page, $add, $rel_auto)
{
    ...
    foreach ($add as $_page) {
        ...
        if ($is_page || !$all_auto) {
            ($fp = fopen($ref_file, 'w')) or
                die_message('cannot write ' . htmlspecialchars($ref_file, ENT_QUOTES, 'UTF-8'));
            fputs($fp, $ref);
            fclose($fp);
        }
    }
