...
function uri_pickup($string = '')
{
    ...
    preg_match_all(
        ...
        '#(\b[a-z][a-z0-9.+-]{1,8}):[/\\\]+' .
            ...
            '\[[0-9a-f:.]+\]' .
            '|' .
            // IPv6([colon-hex and dot]): RFC2732
            '(?:[0-9]{1,3}\.){3}[0-9]{1,3}' .
            '|' .
            // IPv4(dot-decimal): 001.22.3.44
            '[a-z0-9_-][a-z0-9_.-]+[a-z0-9_-]' .
            // hostname(FQDN) : foo.example.org
            ')' .
            '(?::([0-9]*))?' .
            // 4: Port
            '((?:/+[^\s<>"\'\[\]/\#]+)*/+)?' .
            // 5: Directory path or path-info
            '([^\s<>"\'\[\]\#?]+)?' .
            // 6: File?
            '(?:\?([^\s<>"\'\[\]\#]+))?' .
            // 7: Query string
...
function port_normalize($port, $scheme, $scheme_normalize = false)
{
    ...
    $port = intval($port);
    if ($scheme_normalize)
        $scheme = scheme_normalize($scheme);
...
function file_normalize($file = 'index.html.en')
{
    static $simple_defaults = array(
        ...
        'default.aspx' => true,
        'index' => true,
        // Some system can omit the suffix
    );
    ...
    if (isset($simple_defaults[$_file]))
        return '';
    ...
    if ($body == 'index' && $count == 1 && isset($content_suffix[$current]))
        return '';
...
function area_pickup($string = '', $method = array())
{
    ...
    if (isset($method['area_anchor'])) {
        $areas = array();
        $count = isset($method['asap']) ? preg_match($regex, $string) : preg_match_all($regex, $string, $areas);
        if (!empty($count))
            $area['area_anchor'] = $count;
    }
...
{
    if (!is_array($areas) || !is_array($array))
        return;
...
function spam_uri_removing_hocus_pocus($binary = '', $method = array())
{
    ...
    if (is_array($method)) {
        ...
        // '[uri'(4) or ']'(1) or '[/uri]'(6)
        if (
            isset($method['area_anchor']) ||
                isset($method['uri_anchor']) ||
                isset($method['area_bbcode']) ||
                isset($method['uri_bbcode'])
        )
            $length = 1;
    ...
    $string = preg_replace_callback(
        array(
            '#(h?ttp)://' .
                ...
                '/' .
                '([a-z0-9?=&.%_/\'\\\+-]+)' .
                // 3:path/?query=foo+bar+
                '(?:\b|%20)site:([a-z0-9.%_-]+\.[a-z0-9.%_-]+)' .
                // 4:site:nasty.example.com
                '()' .
