...
{
    var $before_bar = '';
    // any number, word ... before bar
    var $after_bar = '';
    // any number, word ... after bar
    var $min_point = '';
    // minimum value
    var $curr_point = '';
    // current value
    ...
    // maximum value
    var $img_compound = '';
    // image to complete bar
    var $img_bg = '';
    // image to complete background
    var $color_compound = '#ff0000';
    // color to complete bar
    var $color_bg = '#c5c5c5';
    // color to complete background
    var $color_border = '#000000';
    // border color, if not specified it doesn't draw border at all
    var $bar_length = '100%';

    // specify length of bar in percentage or in pixels
    ...
    {
        return ($this->max_point - $this->min_point) == 0
            ? sprintf('%.' . $precision . 'f', 0)
            : sprintf('%.' . $precision . 'f', ($this->curr_point / ($this->max_point - $this->min_point)) * 100);
    }
    ...
    function getBar()
    {
        ...
        if ($this->curr_point == $this->min_point) {
            $output .= $this->img_bg
                ? ('<td width="100%" background="' . $this->img_bg . '" bgcolor="' . $this->color_bg . '">&nbsp;</td>')
                : ('<td width="100%" bgcolor="' . $this->color_bg . '">&nbsp;</td>');
        } elseif ($this->curr_point >= $this->max_point) {
            $output .= $this->img_compound
                ? (
                ...
                : ('<td width="100%" bgcolor="' . $this->color_compound . '">&nbsp;</td>');
        } else {
            $output .= $this->img_compound
                ? (
                    '<td width="' .
                ...
                : ('<td width="' . $this->getPercentage(0) . '%" bgcolor="' . $this->color_compound . '">&nbsp;</td>');
            $output .= $this->img_bg
                ? (
                    '<td width="' .
                    (100 - $this->getPercentage(0)) .
                    '%" background="' .
                    $this->img_bg .
                    '" bgcolor="' .
                    $this->color_bg .
                ...
                : (
                    '<td width="' . (100 - $this->getPercentage(0)) . '%" bgcolor="' . $this->color_bg . '">&nbsp;</td>'
                );
        }
        // jo1upk 2006-02-23
        // $output.= "</td></tr>\n";
