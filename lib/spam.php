...
function preg_grep_invert($pattern = '//', $input = array())
{
    static $invert;
    if (!isset($invert))
        $invert = defined('PREG_GREP_INVERT');
...
function strings($binary = '', $min_len = 4, $ignore_space = false, $multibyte = false)
{
    ...
    // Preserve readable spaces if possible

    $binary = $multibyte ? mb_ereg_replace($regex, "\n", $binary) : preg_replace('/' . $regex . '/s', "\n", $binary);
...
{
    if (!is_array($array) || empty($array) && $count_empty)
        return 1;
...
{
    if (preg_match('/^' . '(?:[0-9]{1,3}\.){3}[0-9]{1,3}' . '|' . '(?:[0-9]{1,3}\.){1,3}' . '$/', $string)) {
        return 4;

        // Seems IPv4(dot-decimal)
    } else {
        return 0;
...
function check_uri_spam_method($times = 1, $t_area = 0, $rule = true)
{
    ...
    $positive = array(
        ...
        'quantity' => 8 * $times,
        // Allow N URIs
        'non_uniqhost' => 3 * $times,
        // Allow N duped (and normalized) Hosts
        //'non_uniquri'=>  3 * $times,	// Allow N duped (and normalized) URIs

        // Areas
        'area_anchor' => $t_area,
        // Using <a href> HTML tag
        'area_bbcode' => $t_area,
        // Using [url] or [link] BBCode
    ...
    if ($rule) {
        $bool = array(
            ...
            'uniqhost' => true,
            // Show uniq host (at block notification mail)
            'badhost' => true,
            // Check badhost
        );
    } else {
...
{
    if (!isset($progress['blocked']) || empty($progress['blocked']))
        return '';
    ...
    foreach ($blocked as $list => $lvalue) {
        if (is_array($lvalue) && count($lvalue) == 1 && is_numeric(key($lvalue))) {
