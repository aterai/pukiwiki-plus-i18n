...
function plugin_openid_init()
{
    $msg = array(
        '_openid_msg' => array(
            ...
            'err_store_path' => _(
                'Could not create the FileStore directory %s. Please check the effective permissions.',
            ),
...
function plugin_openid_logoff_msg($author = 'openid', $label = 'OpenID:', $logout_msg = 'logout')
{
    ...
    if (!function_exists('pkwk_session_start'))
        return '<p>' . $_openid_msg['msg_not_found'] . '</p>';
    if (pkwk_session_start() == 0)
        return '<p>' . $_openid_msg['msg_not_start'] . '</p>';
    ...
    if (!empty($name['author']) && $name['author'] !== $author)
        return false;
...
function plugin_openid_inline()
{
    ...
    if (!empty($name['nickname'])) {
        ...
        return (
            sprintf($_openid_msg['msg_logined'], $link) .
            '(<a href="' .
            $cmd .
            '&amp;logout' .
            '">' .
            $_openid_msg['msg_logout'] .
            '</a>)'
        );
    }
...
function plugin_openid_verify($consumer)
{
    ...
    $obj->response = array(
        'server_url' => $auth_request->endpoint->server_url,
...
function plugin_openid_finish_auth($consumer)
{
    ...
    switch ($response->status) {
        ...
        case Auth_OpenID_SUCCESS:
            ...
            $obj->response['local_id'] = !empty($response->endpoint->local_id)
                ? $response->endpoint->local_id
