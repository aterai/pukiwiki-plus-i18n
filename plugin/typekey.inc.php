...
class auth_typekey extends auth_api
{
    ...
    function set_need_email($x)
    {
        $this->need_email = $x;
    }

    function set_version($x)
    {
        $this->version = $x;
    }

    function set_regkeys()
    {
        $this->regkeys = $this->get_regkeys();
    }
    ...
    function get_profile_link()
    {
        ...
        return (
            '<a class="ext" href="' .
            auth_typekey::typekey_profile_url($message['name']) .
            '" rel="nofollow">' .
            $message['nick'] .
            '<img src="' .
            IMAGE_URI .
            'plus/ext.png" alt="" title="" class="ext" onclick="return open_uri(\'' .
            auth_typekey::typekey_profile_url($message['name']) .
            '\',\'_blank\');" /></a>'
        );
    }
...
function plugin_typekey_convert()
{
    ...
    if (!function_exists('pkwk_session_start'))
        return '<p>' . $_typekey_msg['msg_not_found'] . '</p>';
    if (pkwk_session_start() == 0)
        return '<p>' . $_typekey_msg['msg_not_start'] . '</p>';

    if ($auth_api['typekey']['use'] != 1)
        return '<p>' . $_typekey_msg['msg_invalid'] . '</p>';
    if (empty($auth_api['typekey']['site_token']))
        return '<p>' . $_typekey_msg['msg_error'] . '</p>';
...
function plugin_typekey_inline()
{
    ...
    if (!empty($link)) {
        ...
        return (
            sprintf($_typekey_msg['msg_logined'], $link) .
            '(<a href="' .
            auth_typekey::typekey_logout_url($page) .
            rawurlencode('&logout') .
            '">' .
            $_typekey_msg['msg_logout'] .
            '</a>)'
        );
    }
...
function plugin_typekey_get_user_name()
{
    ...
    if (!empty($msg['nick']) && !empty($msg['name'])) {
        return array(
            'role' => ROLE_AUTH_TYPEKEY,
            'name' => $msg['name'],
            'nick' => $msg['nick'],
            'profile' => TYPEKEY_URL_PROFILE . $msg['name'],
            'key' => $msg['name'],
        );
    }
    return array('role' => ROLE_GUEST, 'nick' => '');
