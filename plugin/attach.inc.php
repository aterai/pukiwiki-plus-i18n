...
defined('PLUGIN_ATTACH_UPLOAD_MAX_FILESIZE') or define('PLUGIN_ATTACH_UPLOAD_MAX_FILESIZE', '4M');
// default: 4MB
ini_set('upload_max_filesize', PLUGIN_ATTACH_UPLOAD_MAX_FILESIZE);

// Max file size for upload on script of PukiWikiX_FILESIZE
defined('PLUGIN_ATTACH_MAX_FILESIZE') or define('PLUGIN_ATTACH_MAX_FILESIZE', 2048 * 1024);
// default: 1MB
// 管理者だけが添付ファイルをアップロードできるようにする
defined('PLUGIN_ATTACH_UPLOAD_ADMIN_ONLY') or define('PLUGIN_ATTACH_UPLOAD_ADMIN_ONLY', false);
// FALSE or TRUE
// 管理者だけが添付ファイルを削除できるようにする
defined('PLUGIN_ATTACH_DELETE_ADMIN_ONLY') or define('PLUGIN_ATTACH_DELETE_ADMIN_ONLY', false);
// FALSE or TRUE
// 管理者が添付ファイルを削除するときは、バックアップを作らない
// PLUGIN_ATTACH_DELETE_ADMIN_ONLY=TRUEのとき有効
defined('PLUGIN_ATTACH_DELETE_ADMIN_NOBACKUP') or define('PLUGIN_ATTACH_DELETE_ADMIN_NOBACKUP', false);
// FALSE or TRUE
// アップロード/削除時にパスワードを要求する(ADMIN_ONLYが優先)
defined('PLUGIN_ATTACH_PASSWORD_REQUIRE') or define('PLUGIN_ATTACH_PASSWORD_REQUIRE', false);
// FALSE or TRUE
// 添付ファイル名を変更できるようにする
defined('PLUGIN_ATTACH_RENAME_ENABLE') or define('PLUGIN_ATTACH_RENAME_ENABLE', true);
// FALSE or TRUE
// ファイルのアクセス権
...
// File icon image
define(
    'PLUGIN_ATTACH_FILE_ICON',
    '<img src="' . IMAGE_URI . 'file.png"' . ' width="20" height="20" alt="file"' . ' style="border-width:0px" />',
);

// mime-typeを記述したページ
define('PLUGIN_ATTACH_CONFIG_PAGE_MIME', 'plugin/attach/mime-type');

defined('PLUGIN_ATTACH_UNKNOWN_COMPRESS') or define('PLUGIN_ATTACH_UNKNOWN_COMPRESS', 1);
// 1(compress) or 0(raw)
defined('PLUGIN_ATTACH_COMPRESS_TYPE') or define('PLUGIN_ATTACH_COMPRESS_TYPE', 'TGZ');
...
function attach_upload($file, $page, $pass = null)
{
    ...
    // Check query-string
    $query =
        'plugin=attach&amp;pcmd=info&amp;refer=' . rawurlencode($page) . '&amp;file=' . rawurlencode($file['name']);
    ...
    if (PKWK_QUERY_STRING_MAX && strlen($query) > PKWK_QUERY_STRING_MAX) {
        ...
        PLUGIN_ATTACH_UPLOAD_ADMIN_ONLY &&
            auth::check_role('role_adm_contents') &&
            $pass !== true &&
...
function attach_doupload(&$file, $page, $pass = null, $temp = '', $copyright = false, $notouch = false)
{
    ...
    if ($must_compress && is_uploaded_file($file['tmp_name'])) {
        if (PLUGIN_ATTACH_COMPRESS_TYPE == 'TGZ' && exist_plugin('dump')) {
            ...
            @unlink($tar->filename);
        } else if (PLUGIN_ATTACH_COMPRESS_TYPE == 'GZ' && extension_loaded('zlib')) {
            $obj = &new AttachFile($page, $file['name'] . '.gz');
            if ($obj->exist)
                return array('result' => false, 'msg' => $_attach_messages['err_exists']);

            ($tp = fopen($file['tmp_name'], 'rb')) or die_message(_('The uploaded file cannot be read.')); // アップロードされたファイルが読めません。
        ...
        } else if (PLUGIN_ATTACH_COMPRESS_TYPE == 'ZIP' && class_exists('ZipArchive')) {
            ...
            if ($this->status['freeze']) {
                ...
                if (PLUGIN_ATTACH_RENAME_ENABLE) {
                    $msg_rename =
                        ...
                        '<label for="_p_attach_rename">' .
                        $_attach_messages['msg_rename'] .
                        $msg_require .
                        '</label><br />&nbsp;&nbsp;&nbsp;&nbsp;' .
                        '<label for="_p_attach_newname">' .
                        $_attach_messages['msg_newname'] .
                        ':</label> ' .
    ...
    function toRender($flat)
    {
        ...
        foreach ($files as $file) {
            ...
            $ret .=
                ...
                $cssstyle .
                '">' .
                "{$this->files[$file][0]->size_str}</td>" .
                '<td class="' .
                $cssstyle .
                '">' .
                "{$this->files[$file][0]->type}</td>" .
                '<td class="' .
                $cssstyle .
                '">' .
                "{$this->files[$file][0]->time_str}</td></tr>\n";
        }
        return (
            '<table width="100%" class="attach_table">' .
            "\n" .
            '<tr><th class="attach_th">' .
            $_attach_messages['msg_file'] .
            '</th>' .
            '<th class="attach_th">' .
            $_attach_messages['msg_filesize'] .
            '</th>' .
            '<th class="attach_th">' .
            $_attach_messages['msg_type'] .
            '</th>' .
            '<th class="attach_th">' .
            $_attach_messages['msg_date'] .
            '</th></tr>' .
            "\n$ret</table>\n"
        );
    }
...
class AttachPages
{
    ...
    function AttachPages($page = '', $age = null)
    {
        ...
        $age_pattern = $age === null ? '(?:\.([0-9]+))?' : ($age ? "\.($age)" : '');
