...
//		$sort_last = '0d';
	...
	case '1a': // 1a 初回登録日時(日付順)
		usort($data, function($a,$b) {return $a[1] - $b[1];});
		$color_1st = $bg['cur'];
		$arrow_1st = $_referer_msg['msg_Chr_uarr'];
//		$sort_1st = '1d';
		break;
	case '2d': // 2d カウンタ(大きい順)
		usort($data, function($a,$b) {return $b[2] - $a[2];});
		$color_ctr = $bg['cur'];
		$arrow_ctr = $_referer_msg['msg_Chr_darr'];
		$sort_ctr = '2a';
		break;
	case '2a': // 2a カウンタ(小さい順)
		usort($data, function($a,$b){return $a[2] - $b[2];});
		$color_ctr = $bg['cur'];
		$arrow_ctr = $_referer_msg['msg_Chr_uarr'];
//		$sort_ctr = '2d';
		break;
	case '3': // 3 Referer
		usort($data, function($a,$b) {return ($a[3] == $b[3]) ? 0 : (($a[3] > $b[3]) ? 1 : -1);});
		$color_ref = $bg['cur'];
		break;
	}
