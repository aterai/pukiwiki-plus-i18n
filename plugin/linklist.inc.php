...
// 構成定義ファイル
const CONFIG_REFERER = 'plugin/referer/config';

const CONFIG_LINKLIST = 'plugin/linklist/config';

function plugin_linklist_init(): void
{
    $msg = array(
        '_linklist_msg' => array(
...
function plugin_linklist_action(): array
{
    global $vars;
...
function linklist_analysis($data)
{
    ...
    foreach ($data as $x) {
        ...
        if (empty($url['host']))
            continue;
        if (strpos($url['host'], '.') == '')
            continue; // ホスト名にピリオドが１つもない
        if (linklist_ignore_check($url['host']))
            continue;

        $sw = 0;
        foreach ($IgnoreHost as $y) {
            if (str_contains($url['host'], $y)) {
                $sw = 1;
                continue;
            }
        }
