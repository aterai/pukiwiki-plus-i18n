...
define(
    'AMAZON_XML',
    'http://xml.amazon.co.jp/onca/xml3?t=webservices-20&dev-t=' . AMAZON_DT . '&page=1&f=xml&locale=jp&',
);
...
function plugin_amazon_init()
{
    ...
    $amazon_body = sprintf(
        ...
        _('Author') .
            ...
            "\n" .
            _(
                "// First of all, please delete the full text when you stop this review, and push the [update] button on the page.\n",
            ) .
            ...
            _(
                "// **RECOMMENDATION Above, please do not add a new line. Because it uses it for the contents making.\n",
            ) .
...
function plugin_amazon_convert()
{
    ...
    if ($align == 'clearl')
        return '<div style="clear:left;display:block;"></div>'; // 改行挿入
    if ($align == 'clearr')
        return '<div style="clear:right;display:block;"></div>'; // 改行挿入
    if ($align == 'clear')
        return '<div style="clear:both;"></div>'; // 改行挿入
    if ($align == '')
        $align = 'right';
    if (preg_match('/^(right|left|center)$/', $align) == false)
        return false;
    ...
    if ($check->is_asin) {
        if ($item == 'image' or $item == 'simage') {
            ...
            $div .=
                amazon_get_imagelink($check->asin, $check->ext, $aryargs[3], $info->items['title'], $m_s) . '</div>';
            ...
            if (preg_match('/^((no)?content[sS]?)$/', $item)) {
                ...
                $div .=
                    amazon_get_imagelink($check->asin, $check->ext, $aryargs[3], $info->items['title'], $m_s) .
                    '</div>';
            }
            if ($iscargo) {
                ...
                $div .=
                    ...
                    AMAZON_CARGO .
                    "' name='submit' value='{$_amazon_msg['msg_Cargo']}' /><br />";
            } else
                $div .= "<div class='amazon_sub" . $css_m_s . "' style='text-align:$align'>";
            ...
            $div .=
                "<b>{$_amazon_msg['msg_FixedPrice']}:</b> <del>" .
                $info->items['pricel'] .
                "</del>, <b>{$_amazon_msg['msg_Price']}:</b> " .
                $info->items['price'] .
                "{$_amazon_msg['msg_Tax']}</div>";
            $div .=
                "<div class='amazon_avail" .
                $css_m_s .
            ...
            if ($iscargo)
                $div .= '</div></form>';
            if ($item == 'content')
                $div .= '<br />' . $info->items['content'] . '<div style="display:block;"></div>';
        } elseif ($item == 'delimage') {
            $del_it = unlink(CACHE_DIR . 'ASIN' . $check->asin . '.jpg');
            if (unlink(CACHE_DIR . 'ASIN' . $check->asin . '.gif') and $del_it == false)
                $del_it = true;
            if ($del_it)
                    return "Image of $check->asin deleted...";
                else
                return "Image of $check->asin NOT DELETED...";
        } elseif ($item == 'deltitle') {
            $del_it = unlink(CACHE_DIR . 'ASIN' . $check->asin . '.heavy');
            if (unlink(CACHE_DIR . 'ASIN' . $check->asin . '.lite') and $del_it == false)
                $del_it = true;
            if ($del_it)
                    return "Title of $check->asin deleted...";
                else
                return "Title of $check->asin NOT DELETED...";
        } elseif ($item == 'delete') {
            $del_it = unlink(CACHE_DIR . 'ASIN' . $check->asin . '.heavy');
            if (unlink(CACHE_DIR . 'ASIN' . $check->asin . '.lite') and $del_it == false)
                $del_it = true;
            if (unlink(CACHE_DIR . 'ASIN' . $check->asin . '.jpg') and $del_it == false)
                $del_it = true;
            if (unlink(CACHE_DIR . 'ASIN' . $check->asin . '.gif') and $del_it == false)
                $del_it = true;
            if ($del_it)
                    return "Title and Image of $check->asin deleted...";
                else
                return "Title and Image of $check->asin NOT DELETED...";
        } else {
            if ($item == '') {
                $info = new amazon_getinfo($check->asin, 'heavy');
            ...
            $div .=
                amazon_get_imagelink($check->asin, $check->ext, $aryargs[3], $info->items['title'], 'm') . '</td></tr>';
...
{
    $aryargs = func_get_args();
    if (func_num_args() < 2 or func_num_args() > 5)
            return false;
        ...
        if (
            preg_match('/^(title|author|manufact|media|pricel|price|avail|content|s?image|link|lib0?|key)$/', $item) ==
                false
        )
            return false;
    }
    ...
    if (AMAZON_ALLOW_LNK) {
        if ($item == 'link')
                return amazon_getlink($asin);
            elseif ($item == 'key')
                return amazon_getkey($asin, $aryargs[2], $aryargs[3]); // &amazon(ID, key, Perl スクリプト, books);
            elseif ($item == 'lib0')
                return amazon_getlib($asin, 0);
            elseif ($item == 'lib')
            return amazon_getlib($asin, 1);
    }
...
function amazon_review_save($page, $data)
{
    ...
    if (!is_readable($filename))
        if (amazon_savefile($filename, $data))
            return true;
    return false;
}
...
function amazon_getlib($id, $type)
{
    ...
    $id = trim($id);
    $tmpary = array();
    if (!preg_match('/^([0-9]+)([a-z])?$/', $id, $tmpary))
        return false; // a-z 1 文字はグラフィックパターン
    $id0 = $tmpary[1];
    if ($type == 1)
            $imagelink = '<a href="' . AMAZON_LIB . $id0 . '"><img src="' . CACHE_DIR . 'AZ' . $id . '.gif" ';
        else
        $imagelink = '<a href="' . AMAZON_LIB0 . '"><img src="' . CACHE_DIR . 'AZ' . $id . '.gif" ';
...
function amazon_getkey($id, $key, $media)
{
    ...
    $footer =
        "</iframe><div style='text-align:center'><span style='color:red'>$key0" .
        "{$_amazon_msg['msg_Relation']}<br />($media)</span></div>";
...
function get_keys($str)
{
    ...
    for ($i = 0; $i < strlen($str); $i++) {
        ...
        if ($num >= 0x41 and ($num <= 0x5a or ($num >= 0x61 and $num <= 0x7a)))
                $p0 = get_keys_1($stat, 1, $p0, $i, $str); // a-zA-Z
            elseif ($num >= 0xa1 and $num <= 0xfe) { // JIS X 0208
            $ku = $num - 0xa0;
            $ten = ord(substr($str, $i + 1)) - 0xa0;
            if ($ku == 5 or ($ku == 1 and ($ten == 6 or $ten == 28)))
                    $p0 = get_keys_1($stat, 2, $p0, $i, $str); // 片仮名、ー、・
                elseif ($ku >= 16)
                    $p0 = get_keys_1($stat, 3, $p0, $i, $str); // 漢字
                else
...
class amazon_getinfo
{
    function amazon_getinfo($asin, $which)
    {
        ...
        if ($which != 'lite') {
            ...
            if (!$this->amazon_add_author('|<Author>(.[^<]*)<|U'))
                if (!$this->amazon_add_author('|<Director>(.[^<]*)<|U'))
                    $this->amazon_add_author('|<Artist>(.[^<]*)<|U');
        }
        if ($which != 'heavy') {
    ...
    function amazon_cache_save($asin, $type)
    {
        ...
        if (!is_readable($filename))
                $get_it = 1;
            elseif (($expire * 3600) < (time() - filemtime($filename)))
            $get_it = 1;
        if ($get_it) {
...
class amazon_getimage
{
    function amazon_getimage($asin, $ext, $m_s)
    {
        ...
        if (!is_readable(CACHE_DIR . $filename))
                $get_it = 1;
            elseif ((AMAZON_EXPIRE_img * 3600) < (time() - filemtime(CACHE_DIR . $filename)))
            $get_it = 1;
        if ($get_it) {
            if ($use_proxy)
                    $rc = http_request(AMAZON_IMAGE . "$asin.$ext.$fileext"); // Thanks to upk
                else
                $rc['data'] = amazon_getfile(AMAZON_IMAGE . "$asin.$ext.$fileext");
            if ($this->amazon_getimagesize($rc['data'], $asin) <= 1) { // 通常は 1 が返る (reimy)
                // キャッシュを NO_IMAGE のコピーとする
                if ($ext == '09') {
                    if ($use_proxy)
                            $rc = http_request(AMAZON_IMAGE . "$asin.01.$fileext");
                        else
                        $rc['data'] = amazon_getfile(AMAZON_IMAGE . "$asin.01.$fileext");
            ...
            if (!amazon_savefile(CACHE_DIR . $filename, $rc['data']))
                return false;
        }
        $this->file = $filename;
    }
    ...
    function amazon_getimagesize($body, $asin)
    {
        ...
        if (!amazon_savefile($tmpfile, $body))
            return 0;
