...
class CodeHighlight
{
    ...
    function highlight(&$lang, &$src, &$option)
    {
        ...
        if ($option['menu']) {
            ...
            if ($option['outline']) {
                ...
                $menu .=
                    '<img src="' .
                    PLUGIN_CODE_OUTLINE_OPEN_FILE .
                    '" style="cursor: hand" alt="' .
                    $_code_expand .
                    '" title="' .
                    $_code_expand .
                    '" ' .
                    'onclick="javascript:code_all_outline(\'' .
                    PLUGIN_CODE_HEADER .
                    $id_number .
                    '\',' .
                    $data['blocknum'] .
                    ...
                    IMAGE_DIR .
                    '\')" ' .
                    'onkeypress="javascript:code_all_outline(\'' .
                    CODE_HEADER .
                    $id_number .
                    '\',' .
                    $data['blocknum'] .
                ...
                $menu .=
                    '<img src="' .
                    PLUGIN_CODE_OUTLINE_CLOSE_FILE .
                    '" style="cursor: hand" alt="' .
                    $_code_short .
                    '" title="' .
                    $_code_short .
            ...
            if ($option['comment']) {
                // コメントの開閉ボタン
                $menu .=
                    '<input type="button" value="comment open" ' .
                    'onclick="javascript:code_comment(\'' .
                    PLUGIN_CODE_HEADER .
                    $id_number .
                    '\',' .
                    $data['commentnum'] .
                    ',\')" ' .
                    'onkeypress="javascript:code_comment(\'' .
                    PLUGIN_CODE_HEADER .
                    $id_number .
                    '\',' .
                    $data['commentnum'] .
                    ',\')" />';
                $menu .=
                    '<input type="button" value="comment close" ' .
                    ' onclick="javascript:code_comment(\'' .
                    PLUGIN_CODE_HEADER .
                    $id_number .
                    '\',' .
    ...
    function lineToHTML(&$string, &$lang, $id_number, &$option)
    {
        ...
        while ($line !== false) {
            ...
            switch ($switchHash[$line[0]]) {
                ...
                case PLUGIN_CODE_COMMENT_CHAR:
                    ...
                    if ($option['link'])
                        $line = preg_replace(
                            '/(s?https?:\/\/|ftp:\/\/|mailto:)([-_.!~*()a-zA-Z0-9;\/:@?=+$,%#]|&amp;)+/',
                            '<a href="$0">$0</a>',
                            $line,
                        );
                    $html .=
                ...
                case PLUGIN_CODE_IDENTIFIRE_CHAR:
                    ...
                    if ($index != '')
                            $html .=
                                '<span class="' . PLUGIN_CODE_HEADER . $code_css[$index - 1] . '">' . $line . '</span>';
                        else
                        $html .= $line;
                ...
                case PLUGIN_CODE_MULTILINE:
                    ...
                    if ($index != '')
                            $html .=
                                '<span class="' . PLUGIN_CODE_HEADER . $code_css[$index - 1] . '">' . $src . '</span>';
                        else
                        $html .= $src;
                    continue 2;
        ...
        if ($mknumber)
            $html['number'] = _plugin_code_makeNumber($num_of_line - 2); // 最後に改行を削除したため -2
        return $html;
    }
                    ...
                    if ($nest <= $array['nest']) {
                        ...
                        if ($plus == '')
                            $plus =
                                '<a class="' .
                                PLUGIN_CODE_HEADER .
                                'outline" href="javascript:code_outline(\'' .
                                PLUGIN_CODE_HEADER .
                                $id .
                                '\',\'' .
                                IMAGE_DIR .
...
function _plugin_code_multiline_argment(&$arg, &$data, &$option, $begin = 0, $end = null)
{
    ...
    if ($arg[strlen($arg) - 1] != "\n") {
        ...
        if (PLUGIN_CODE_FILE_ICON && !$option['noicon'] || $option['icon'])
                $icon = FILE_ICON;
            else
            $icon = '';
...
function _plugin_code_read_file_data(&$name, $begin = 0, $end = null)
{
    ...
    if ($is_url) { // URL
        ...
        $url = $script . '?plugin=attach' . '&amp;refer=' . rawurlencode($page) . '&amp;openfile=' . rawurlencode($name); // Show its filename at the last
    }

    $params['url'] = $url;
    $params['info'] =
        get_date('Y/m/d H:i:s', filemtime($file) - LOCALZONE) .
        ' ' .
        sprintf('%01.1f', round(filesize($file) / 1024, 1)) .
        'KB';
...
function _plugin_code_column(&$text, $number = null, $outline = null)
{
    ...
    if (PLUGIN_CODE_TABLE) {
        $html .= '<table class="' . PLUGIN_CODE_HEADER . 'table" border="0" cellpadding="0" cellspacing="0"><tr>';
        if ($number !== null)
            $html .= '<td>' . $number . '</td>';
        if ($outline !== null)
            $html .= '<td>' . $outline . '</td>';
