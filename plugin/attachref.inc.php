...
function plugin_attachref_init()
{
    $messages = array(
        '_attachref_messages' => array(
            ...
            'msg_collided' => _(
                'It seems that someone has already updated the page you were editing.<br />The attach file was added, alhough it may be inserted in the wrong position.<br />',
            ),
        ),
    );
...
function attachref_form($page)
{
    ...
    $f_no = isset($vars['attachref_no']) && is_numeric($vars['attachref_no']) ? ($vars['attachref_no'] + 0) : 0;
