...
// 検索エンジン
// Google
defined('SKEYLIST_SEARCH_URL') or
...
function skeylist_analysis($data)
{
    ...
    foreach ($data as $x) {
        ...
        $url = parse_url($x[3]);
        if (empty($url['query']))
            continue; // queryストリングが空の場合は対象外

        // queryストリングの解析
        $tok = strtok($url['query'], '&');
        while ($tok) {
            list($key, $parm) = explode('=', $tok); // キーと値に分割
            $tok = strtok('&'); // 次の処理の準備
            ...
            if (empty($parm))
                continue; // 値が入っていない場合
            if (strpos($parm, 'cache:') === 0)
                continue; // google のキャッシュなどの場合
