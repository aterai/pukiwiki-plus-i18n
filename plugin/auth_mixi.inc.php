...
function plugin_auth_mixi_inline()
{
    ...
    // 認証済みの扱い
    $icon =
        '<img src="' . IMAGE_URI . 'plus/openid/mixi/icon.gif"' . ' width="16" height="16" alt="mixi" title="mixi" />';
    ...
    if ($parm['type'] == 'friends' && empty($parm['id'])) {
        ...
        $redirect_url = get_cmd_uri('openid', $page, '', array(
            'action' => 'verify',
            'openid_url' => $idp,
            'author' => 'auth_mixi',
        ));
...
function auth_mixi_set_loginuri($type = '', $icon = 1, $id = '')
{
    ...
    switch ($type) {
        case 'friends':
            // マイミク認証
            if (empty($id))
                $id = PLUGIN_AUTH_MIXI_MY_ID;
            if (empty($id))
                $die_message('マイミク認証時は、IDの設定が必須です。');
            $idp = 'https://id.mixi.jp/' . $id . '/friends';
            break;
        case 'community':
            // コミュニティ認証
            if (empty($id))
                $die_message('コミュニティ認証時は、コミュニティIDの設定が必須です。');
...
function auth_mixi_get_info($openid)
{
    ...
    switch (count($id)) {
        case 3:
            ...
            } else if ($id[0] === 'community') { // auth community
                // 0 : auth community (3)
                // 1 : community id
                // 2 : user id
...
function auth_mixi_get_role_auth_api($type, $id)
{
    ...
    if (!isset($config_mixi[$type])) {
        ...
        if (!is_array($config_mixi[$type]))
            $config_mixi[$type] = explode(',', $config_mixi[$type]);
        if ($type === 2)
            $config_mixi[2][] = PLUGIN_AUTH_MIXI_MY_ID;
    }
