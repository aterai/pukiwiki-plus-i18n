...
function plugin_recent_action()
{
    ...
    if ($prev_lines > 0) {
        $prev_link =
            "<a class=\"page-link\" href=\"$script?plugin=recent" .
            "&amp;offset=$prev_offset&amp;lines=$prev_lines\" aria-label=\"Previous\"><span aria-hidden=\"true\">«</span></a>";
    }
    if ($next_lines > 0) {
        $next_link =
            "<a class=\"page-link\" href=\"$script?plugin=recent" .
            "&amp;offset=$next_offset&amp;lines=$next_lines\" aria-label=\"Next\"><span aria-hidden=\"true\">»</span></a>";
    }
    if ($total_page > 1) {
        for ($i = 0; $i < $total_page;) {
            ...
            if ($i != $now_page) {
                $page_link .=
                    "<li><a class=\"page-link\" href=\"$script?plugin=recent" .
                    "&amp;offset=$page_offset&amp;lines=$lines\">$page_num</a></li>";
            } else {
