 ...
 * @version     $Id: jugemkey.inc.php,v 0.15 2009/12/13 15:33:00 upk Exp $
...
function plugin_jugemkey_convert()
{
    ...
    if (!function_exists('pkwk_session_start'))
        return '<p>' . $_jugemkey_msg['msg_not_found'] . '</p>';
    if (pkwk_session_start() == 0)
        return '<p>' . $_jugemkey_msg['msg_not_start'] . '</p>';
...
function plugin_jugemkey_inline()
{
    ...
    if (!function_exists('pkwk_session_start'))
        return $_jugemkey_msg['msg_not_found'];
    if (pkwk_session_start() == 0)
        return $_jugemkey_msg['msg_not_start'];
    ...
    if (isset($name['title'])) {
        ...
        return (
            sprintf($_jugemkey_msg['msg_logined'], $link) .
            '(<a href="' .
            $logout_url .
            '">' .
            $_jugemkey_msg['msg_logout'] .
            '</a>)'
        );
    }
...
function plugin_jugemkey_action()
{
    ...
    if (isset($vars['userinfo'])) {
        ...
        $body =
            '<h3>' .
            $_jugemkey_msg['msg_userinfo'] .
            '</h3>' .
            '<strong>' .
            $_jugemkey_msg['msg_user_name'] .
            ': ' .
            $rc['title'] .
            '</strong>';
...
function plugin_jugemkey_get_user_name()
{
    ...
    if (!empty($msg['title']))
        return array('role' => ROLE_AUTH_JUGEMKEY, 'nick' => $msg['title'], 'profile' => $info, 'key' => $msg['title']);
    return array('role' => ROLE_GUEST, 'nick' => '');
}
