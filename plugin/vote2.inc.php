...
defined('VOTE2_COOKIE_EXPIRED') or define('VOTE2_COOKIE_EXPIRED', 60 * 60 * 24 * 3);
// 連続投票禁止時間
defined('VOTE2_COLOR_BG') or define('VOTE2_COLOR_BG', '#d0d8e0');
// 棒グラフの背景色
defined('VOTE2_COLOR_BORDER') or define('VOTE2_COLOR_BORDER', '#ccd5dd');
// 棒グラフの枠色
defined('VOTE2_COLOR_BAR') or define('VOTE2_COLOR_BAR', '#0000ff');
...
function plugin_vote2_init()
{
    $messages = array(
        '_vote2_messages' => array(
            ...
            'update_failed' => _(
                'Vote failure: In $1, there was the address of the vote or the item was not corresponding.',
            ),
...
function plugin_vote2_inline()
{
    ...
    $page = $vars['page'];
    if (!array_key_exists($page, $numbers))
        $numbers[$page] = 0;
    ...
    foreach ($args as $opt) {
        ...
        if (preg_match('/^(.+(?==))=([+-]?\d+)([ibr]?)$/', $opt, $match)) {
            list($page, $vote_inno, $f_vote_inno, $ndigest) = plugin_vote2_address($match, $vote_inno, $page, $ndigest);
            continue;
        }
...
function plugin_vote2_address($match, $vote_no, $page, $ndigest)
{
    ...
    switch ($vote2_attr_arg) {
        ...
        case 'i':
        case 'b':
            $vote_no = $vote2_no_arg . $vote2_attr_arg;
            break;
        case 'r':
            if ($this_flag) {
