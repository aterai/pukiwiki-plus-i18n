...
function plugin_dav_makemultistat($tree, $path, $depth)
{
    ...
    // !=1 は、ファイル名の場合は、それを除外
    // dir の場合に処理する
    if (is_array($tree) && preg_match('/\/$/', $path) != 1)
        return null;
...
function plugin_dav_error_exit($code, $msg = null, $url = null)
{
    ...
    if (!array_key_exists($code, $array_msg))
        $code = 500;
...
function plugin_dav_maketree($path)
{
    ...
    if (isset($page) && !check_readable($page, false, false))
        plugin_dav_error_exit(401); // 'user/password'
