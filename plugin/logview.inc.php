        ...
        foreach ($view as $field) {
            switch ($field) {
                ...
                case '@guess': // 推測
                    $body .=
                        ' <td class="style_td">' .
                        htmlspecialchars(logview_guess_user($data, $guess), ENT_QUOTES) .
                        "</td>\n";
                    break;
                ...
                case 'ua': // ブラウザ情報 (USER-AGENT)
                    ...
                    if (!empty($browser)) {
                        $body .=
                            ...
                            '.png"' .
                            ' alt="' .
                            htmlspecialchars($data['ua'], ENT_QUOTES) .
                            '" title="' .
                            htmlspecialchars($data['ua'], ENT_QUOTES) .
                            '" />';
                    }
...
function logview_guess_user($data, $guess)
{
    ...
    if (!isset($guess[$data['ua']]))
        return ''; // USER-AGENT が一致したデータがあるか
    if (!isset($data['host']))
        return '';
...
function logview_user_list(&$fld, $page, $kind)
{
    ...
    $ret .=
        '<div><fieldset>' .
        $_logview_msg['all_user'] .
        ': ' .
        $all_user_idx .
        ' ' .
        $_logview_msg['number_unused'] .
        ': ' .
        $ctr .
        ' ' .
        $_logview_msg['availability'] .
        ': ' .
        floor(100 - (($ctr / $all_user_idx) * 100)) .
