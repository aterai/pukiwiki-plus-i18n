...
defined('LIVEDOOR_PERMS') or define('LIVEDOOR_PERMS', 'id');
// userhash or id
...
function plugin_livedoor_convert()
{
    ...
    if (!function_exists('pkwk_session_start'))
        return '<p>' . $_livedoor_msg['msg_not_found'] . '</p>';
    if (pkwk_session_start() == 0)
        return '<p>' . $_livedoor_msg['msg_not_start'] . '</p>';
...
function plugin_livedoor_inline()
{
    ...
    if (isset($name['livedoor_id'])) {
        ...
        return (
            sprintf($_livedoor_msg['msg_logined'], $name['livedoor_id']) .
            '(<a href="' .
            $logout_url .
            '">' .
            $_livedoor_msg['msg_logout'] .
            '</a>)'
        );
    }
...
function plugin_livedoor_get_user_name()
{
    ...
    if (!empty($msg['livedoor_id']))
        return array(
            'role' => ROLE_AUTH_LIVEDOOR,
            'nick' => $msg['livedoor_id'],
            'key' => $msg['livedoor_id'],
            'profile' => $info,
        );
    return array('role' => ROLE_GUEST, 'nick' => '');
