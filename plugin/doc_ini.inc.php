...
function plugin_doc_ini_init()
{
    $msg = array(
        '_doc_ini_msg' => array(
            'title_confirmation' => _('Confirmation of page initialization'),
            // ページ初期化の確認
            'msg_confirmation' => _('May I really initialize the page of %s'),
            // %s のページを本当に初期化してもよろしいでしょうか？
            'title_end' => _('Initialization ended'),
            // 初期化終了
...
function plugin_doc_ini_convert()
{
    ...
    if (empty($vars['page']))
        return '';
    if (!doc_ini_file_exist($vars['page']))
        return '';
...
function plugin_doc_ini_exec($page)
{
    ...
    $msg_body = '<ul>';
    if (!$backup)
        $msg_body .= '<li>' . $_doc_ini_msg['msg_abend_backup'] . '</li>';
    if (!$diff)
        $msg_body .= '<li>' . $_doc_ini_msg['msg_abend_diff'] . '<li>';
    $msg_body .= '</ul>';
    return array('msg' => $_doc_ini_msg['title_end'], 'body' => $msg_body);
}
