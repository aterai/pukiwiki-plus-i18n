...
function plugin_tb_save($url, $tb_id)
{
    ...
    static $fields = array('url', 'title', 'excerpt', 'blog_name'); /* UTIME, */
...
function plugin_tb_mode_view_set($page)
{
    ...
    foreach ($data as $x) {
        if (count($x) != 5)
            continue; // Ignore incorrect record
    ...
    $body .=
        '<div style="text-align:right">' .
        '<a href="' .
        get_cmd_uri('tb', '', '', '__mode=view') .
        '">' .
        '<img src="' .
        IMAGE_URI .
        'plus/trackback.png" alt="" title="" />' .
        'Trackback List' .
        '</a>' .
        "</div>\n";
