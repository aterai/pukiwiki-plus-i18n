 ...
 * ブロック型プラグイン処理
 */
function plugin_toc_convert(): string
{
    global $vars;
    ...
    if (empty($lvl))
        $lvl = 3; // 表示レベル : 3 まで表示
    if (empty($view))
        $view = 'toc'; // 表示形式   : toc, tree
    if (empty($mode))
        $mode = 'toc'; // 処理対象   : full, toc, part
    if (empty($id))
        $id = ''; // #toc 位置識別子
...
function toc_from_to_check($lvl, $dat_lvl): int
{
    $chk_lvl = abs($lvl);
 ...
 * インデックスの把握
 */
function toc_make_index($src, $mode, $lvl): array
{
    $rc = array();
    $i = -1;
    $sw = $mode == 'toc' ? 0 : 1;
    ...
    foreach ($src as $_src) {
        ...
        if ($sw == 0) {
            if (str_starts_with($_src, '#toc(') || trim($_src) == '#toc') {
                $sw = 1;
                continue;
            }
        }
        ...
        if (!$sw)
            continue;
        if (!toc_from_to_check($lvl, $dat_lvl))
            continue;
 ...
 * パラメータ解析
 */
function toc_get_id($name, $data, $no): string
{
 ...
 * 明細編集(階層表示なし)
 */
function toc_mode_toc($idx): string
{
    $hed = '#content_1_';
    $sw = 1;
    $rc = '';
 ...
 */
function toc_mode_contents($idx): string
{
    static $seq = 0;
    ...
    $format =
        '<ul class="list%s" style="padding-left:%spx;margin-left:%spx">' . '<li><a href="%s">%s</a></li></ul>' . "\n";
...
function toc_convert_index($idx, $lvl): array
{
