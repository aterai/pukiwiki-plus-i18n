...
function plugin_blikifooter_convert()
{
    ...
    if (!empty($args[0])) {
        $retval .= $_blikifooter_msg['msg_author'] . '[[' . $args[0] . ']]' . $_blikifooter_msg['seplater'];
    }
    ...
    if ($trackback && isset($args[1]) && $args[1] != '0') {
        $retval .=
            $_blikifooter_msg['seplater'] .
            '[[' .
            $_blikifooter_msg['msg_trackback'] .
            '(' .
            tb_count($vars['page']) .
            '):' .
            $script .
    ...
    if (!is_null($comment_count)) {
        $retval .= $_blikifooter_msg['seplater'] . sprintf($_blikifooter_msg['msg_comment'], $comment_count);
    }
