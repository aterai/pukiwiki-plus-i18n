...
class auth_hatena extends auth_api
{
    ...
    function get_profile_link()
    {
        ...
        return (
            '<a class="ext" href="' .
            auth_hatena::hatena_profile_url($message['name']) .
            '" rel="nofollow">' .
            $message['name'] .
            '<img src="' .
            IMAGE_URI .
...
function plugin_hatena_convert()
{
    ...
    if (!$auth_api['hatena']['use'])
        return '<p>' . $_hatena_msg['msg_invalid'] . '</p>';

    if (!function_exists('pkwk_session_start'))
        return '<p>' . $_hatena_msg['msg_not_found'] . '</p>';
    if (pkwk_session_start() == 0)
        return '<p>' . $_hatena_msg['msg_not_start'] . '</p>';
...
function plugin_hatena_inline()
{
    ...
    if (isset($name['name'])) {
        ...
        $logout_url .= '&amp;logout';
        return (
            sprintf($_hatena_msg['msg_logined'], $link) .
            '(<a href="' .
            $logout_url .
            '">' .
            $_hatena_msg['msg_logout'] .
            '</a>)'
        );
    }
...
function plugin_hatena_get_user_name()
{
    ...
    if (!empty($msg['name']))
        return array(
            'role' => ROLE_AUTH_HATENA,
            'nick' => $msg['name'],
            'profile' => HATENA_URL_PROFILE . $msg['name'],
            'key' => $msg['name'],
        );
    return array('role' => ROLE_GUEST, 'nick' => '');
}
