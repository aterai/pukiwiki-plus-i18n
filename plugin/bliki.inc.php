...
function plugin_bliki_init()
{
    ...
    $msg = array(
        '_bliki_msg' => array(
            'msg_more' => _('&size(12){->[[The continuation of %s is read.:%s]]};'),
            // 「もっと読む」リンクの文言
            'msg_update' => _('UPDATE'),
        ),
    );
...
function plugin_bliki_convert()
{
    ...
    list($child, $cache_name, $recent_prefix, $page_num) = bliki_process_args(func_get_args());
...
function plugin_bliki_action()
{
    ...
    if ($cache == 'DELALL') {
        ...
        return array('result' => false, 'msg' => 'bliki caches are cleared.', 'body' => 'bliki caches are cleared.');
    }
}
...
function bliki_get_pages($recent_prefix, $child, $cache_name)
{
    ...
    foreach ($lines as $line) {
        ...
        if ($recent_prefix != '' && strpos($page, $recent_prefix) !== 0)
            continue;
        if (!$child && strpos($page, '/', strlen($recent_prefix)) !== false)
            continue;
        if (!check_readable($page, false, false))
            continue;

        array_push($pages, array('time' => $time, 'page' => $page));
...
function bliki_convert_html($entries, $page_num)
{
    ...
    for ($cnt = 0; $cnt < sizeof($entries) && $cnt < $page_num; ++$cnt) {
        ...
        $contents .= sprintf(BLIKI_PAGE_TEMPLATE, convert_html($entries[$cnt]['entry']));
        $vars['page'] = $page_back;
    }
...
function bliki_get_more($page)
{
    ...
    return sprintf($_bliki_msg['msg_more'], htmlspecialchars($page), get_page_uri($page, '', '', 'more'));
}
