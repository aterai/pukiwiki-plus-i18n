...
function plugin_dump_download()
{
    ...
    $filecount = 0;
    $tar = new tarlib();
    $tar->create(CACHE_DIR, $arc_kind) or die_message(_('It failed in the generation of a temporary file.'));
...
function plugin_dump_upload()
{
    ...
    if (!preg_match('/(\.tar|\.tar.gz|\.tgz)$/', $filename, $matches)) {
        ...
        switch ($matches[1]) {
            ...
            case '.tar.gz':
                ...
                break;
            default:
                die_message(_('Invalid file suffix: ') . $matches[1]);
        }
    }
    ...
    $pattern =
        "(({$_STORAGE['DATA_DIR']['extract_filter']})|" .
...
function plugin_dump_disp_form()
{
    ...
    $_dump_namedecode =
        _('Virtual of page name encode is converted into the file name with the directory.') .
