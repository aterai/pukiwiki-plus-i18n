...
define(
    'PLUGIN_MINICALENDAR_VIEWER_DATE_FORMAT',
...
define('PLUGIN_MINICALENDAR_VIEWER_USAGE', '#calendar_viewer(pagename,this|yyyy-mm|n|x*y[,mode[,separater]])');
...
function plugin_minicalendar_viewer_convert()
{
    ...
    if (
        isset($func_vars_array[2]) &&
            preg_match('/^(past|pastex|view|viewex|future|futureex)$/si', $func_vars_array[2])
    ) {
        ...
        if (PLUGIN_MINICALENDAR_VIEWER_COMMENT === true) {
            if (is_page(':config/plugin/addline/comment') && exist_plugin_inline('addline')) {
                ...
                $tail .= str_replace(
                    ...
                    '><img src="' .
                        IMAGE_URI .
                        'plus/comment.png" width="15" height="15" alt="Comment" title="Comment" />Comment',
                    $comm,
                );
            }
        }
        if (PLUGIN_MINICALENDAR_VIEWER_TRACKBACK === true) {
            if ($trackback) {
                ...
                $tail .=
                    '<a href="' .
                    $script .
                    '?plugin=tb&amp;__mode=view&amp;tb_id=' .
                    $tb_id .
                    '">' .
                    '<img src="' .
                    IMAGE_URI .
                    'plus/trackback.png" width="15" height="15" alt="" title="" />Trackback(' .
                    tb_count($page) .
                    ')' .
                    '</a>' .
                    "\n";
    ...
    if ($left_YM != '') {
        if ($left_YMX != '') {
            ...
            $left_link =
                '<a href="' .
                $script .
                '?plugin=minicalendar_viewer&amp;file=' .
                $enc_pagename .
                '&amp;date=' .
                $left_YM .
                '&amp;date_sep=' .
