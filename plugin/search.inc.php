...
function plugin_search_action()
{
    ...
    if ($s_word != '') {
        ...
        header('Location: ' . get_script_uri() . '?cmd=search&id=' . $r_id . '&word=' . $r_word);
        exit();
    } else {
        // Init
        unset($vars['word']); // Stop using $_msg_word at lib/html.php
...
function plugin_search_display_format($highlight, $id = '', $start = 0, $len = 10)
{
    ...
    if (
        $id === '' ||
            !isset($session_value['time']) ||
            $session_value['time'] != $id ||
            $session_value['word'] != $highlight
    ) {
    ...
    $retval .= str_replace(
        '$1',
        $s_word,
        str_replace('$2', $found_pages, str_replace('$3', $total_pages, $b_type ? $_msg_andresult : $_msg_orresult)),
    );
    ...
    if ($found_pages > $len) {
        ...
        $retval .= str_replace('$1', $pos_start, str_replace('$2', $pos_end, $_msg_foundnavigator));
...
function plugin_search_makelink($id, $word, $start, $len, $label, $title, $now = '')
{
    ...
    if ($now !== '' && $start == $now) {
        $body =
            '<li class="page-item active"><a class="page-link" href="#"><span title="' .
            $title .
            '"><strong>' .
            $s_label .
            '</strong></span> <span class="sr-only">(current)</span></a></li>';
    } else {
        $body =
