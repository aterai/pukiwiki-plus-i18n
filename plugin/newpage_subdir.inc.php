    ...
    foreach ($roots as $root) {
        ...
        foreach ($pages as $page) {
            ...
            if (strpos($page, $root) === 0) {
                if (
                    isset($option['directory only']) &&
                        $option['directory only'] &&
                        strrpos($page, '/') >= strlen($root)
                ) {
        ...
        if (!$matched) {
            ...
            $warnings[] =
                '<font color="red">' .
                sprintf(_("#%s doesn't have the corresponding page. "), $root) .
                '</font>' .
                '(<a href="' .
                get_page_uri($root) .
                '">' .
...
function print_help_message()
{
    return (
        ...
        _("OPTION<br />\n") .
        _(
            "　-d Directory Only.	It limits it only to the one with the child page. (The directory specified specifying it is an exception. )<br />\n",
        ) .
