...
function plugin_adm_auth_wkgrp_init()
{
    $msg = array(
        '_adm_auth_wkgrp_msg' => array(
            ...
            'msg_chk_1' => _(
                "The following images are the one (reading auth_wkgrp, and conversion into page image).\n",
            ),
        ),
    );
...
function plugin_adm_auth_wkgrp_convert()
{
    ...
    if (is_page($config_page_name)) {
        ...
        $guide_msg = $filetime_config_page > $filetime_auth_wkgrp
            ? ('<strong>' . $_adm_auth_wkgrp_msg['msg_gen'] . '</strong>')
            : '';
...
function adm_auth_wkgrp_put_file($wkgrp_user)
{
    ...
    foreach ($wkgrp_user as $name => $api_val) {
        ...
        foreach ($api_val as $id => $val) {
            fputs(
                $fp,
                "\t\t'$id'\t=> array('role'=>{$val['role']}, 'displayname'=>'{$val['displayname']}', 'group'=>'{$val['group']}', 'mypage'=>'{$val['mypage']}', 'home'=>'{$val['home']}'),\n",
            );
        }
        fputs($fp, "\t),\n");
    }
