...
function plugin_cvscheck_convert()
{
    ...
    if ($acvs == '')
        $acvs = plugin_cvscheck_cvsversions();
    if ($acvs == '' or !count($acvs))
        return false;
    if ($cache_write_flag)
        plugin_cvscheck_cache_write($cache_file, $acvs);
    if ($local_flag == 0) {
...
function plugin_cvscheck_diff($alocal, $acvs)
{
    ...
    foreach ($acvs as $site => $ary) {
        foreach ($ary as $file => $cver) {
            if (!array_key_exists($file, $alocal[$site]))
                continue;
            $lver = $alocal[$site][$file];
            if ($lver == '' or $lver == $cver)
                continue;
...
function plugin_cvscheck_diff_local($alocal, $acvs, $optional_flag, $interval)
{
    ...
    if ($optional_flag) {
        ...
        if ($aopt == '')
            $aopt = plugin_cvscheck_localoptional();
...
function plugin_cvscheck_cvsversions()
{
    ...
    foreach ($sites as $key => $site) {
        ...
        if (!preg_match_all('/<tr(?>[^>])*>((?>.(?!<\/tr>))+.)<\/tr>/is', $html, $matches))
            continue;

        $ary = array();
        foreach ($matches[1] as $line) {
            if (!preg_match_all('/<td(?>[^>])*>((?>.(?!<\/td>))+.)<\/td>/is', $line, $match))
                continue;
            if (count($match[1]) < 3)
                continue;
            $file = htmlspecialchars(trim(preg_replace('/<[^>]+>|&\w+;/', '', $match[1][0])));
            $ver = htmlspecialchars(trim(preg_replace('/<[^>]+>|&\w+;/', '', $match[1][2])));
            if (!preg_match('/^[\w\.]+$/', $file) or !preg_match('/^[\d\.]+$/', $ver))
                continue;
    ...
    foreach ($dirs as $key => $adir) {
        ...
        while ($file = $dir->read()) {
            ...
            if (preg_match('/\$' . 'Id: (.+),v ([\d\.]+) (\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2})/', $data, $matches)) {
                ...
                if (plugin_cvscheck_greaterp($ver, $ary[$file]))
                    $ary[$file] = $ver;
...
function plugin_cvscheck_localoptional()
{
    ...
    if ($ret['rc'] != 200)
        return $outs;
    $html = $ret['data'];
    if (!preg_match_all("/$dir([\w\.]+)\"/is", $html, $match))
        return $outs;
