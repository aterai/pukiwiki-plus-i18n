...
class XHTML2Wiki
{
    ...
    function Div($line)
    {
        ...
        if (preg_match('/<pre>/', $line, $matches)) {
        ...
        } else if (preg_match('/<h([2-4])(.*?)>(.*)/', $line, $matches)) { // 見出し
            ...
            $this->EndDiv();
        } else if (preg_match("/<div\s([^>]*?)class=\"(plugin|ref)\"(.*?)>(.*)/", $line)) { // ブロック型プラグイン
            $line = preg_replace("/<br\s\/>/", "\n", $line);
            $line = strip_tags($line);
            $this->OutputLine($this->DecodeSpecialChars($line));
        } else if (preg_match('/<blockquote.*?>/', $line)) { // 引用文
            if ($this->GetDiv() != 'Blockquote') {
                $this->StartDiv('Blockquote');
            }
            $this->div_level++;
            $this->OutputLine(str_repeat('>', $this->GetLevel()));
        } else if (preg_match('/<(o|u|d)l.*?>/', $line, $matches)) { // リスト
            $element = strtoupper($matches[1]) . 'List';
            if ($this->GetDiv() != $element) {
                $this->StartDiv($element);
            }
            $this->div_level++;
        } else if (preg_match('/<table.*?>/', $line)) { // テーブル
            $this->StartDiv('Table');
        } else if (preg_match("/^<hr(\sclass=\"(full_hr|short_line)\")?\s\/>$/", $line, $matches)) { // 水平線
            if ($matches[2] == 'short_line') {
                $this->OutputLine('#hr');
            } else {
                $this->OutputLine('----');
            }
        } else if (strpos($line, '<div style="page-break-after: always;">') !== false) { // 改頁
    ...
    function Table($line)
    {
        ...
        if ($is_cell) {
        ...
        } else if (preg_match('/<tr.*?>/', $line)) { // UPK // 行の開始
            $col = 1;
            $row++;
            $delm = preg_match('/<tr>/', $line) ? '|' : ',';
        } else if (preg_match("/<\/table>/", $line)) { // テーブルの終了
            $cells = null;
            $this->EndDiv();
            $this->body[] = "\n";
        } else if (preg_match('/<colgroup>/', $line)) { // 書式設定行
            if (count($cells)) {
            ...
            $this->body[] = $text . "|c\n";
        } else if (preg_match('/<t((h)ead|body|(f)oot)>/', $line, $matches)) { // ヘッダ・ボディ・フッタの開始
            $cells = array();
            $type = !empty($matches[2]) ? $matches[2] : (!empty($matches[3]) ? $matches[3] : '');
            $row = 0;
        } else if (preg_match("/<\/t(head|body|foot)>/", $line)) { // ヘッダ・ボディ・フッタの終わり
    ...
    function Image($matches)
    {
        ...
        if (preg_match("/alt=\"(.*?)\"/", $attribute, $matches)) {
            ...
            } else if ($alt == 'Comment' && preg_match("/title=\"(.+?)\"/", $attribute, $matches)) { // コメント
                $comment = $matches[1];
            ...
            } else if (preg_match("/^\[(.+)\]$/", $alt, $matches)) { // else if (preg_match("/^\[.+\]$/", $alt, $matches)) { // JO1UPK // 顔文字
                return '&' . $matches[1] . ';';
            }
            return ' ' . $alt . ' ';
        }
