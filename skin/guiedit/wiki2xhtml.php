...
class HeadingEx extends ElementEx
{
    ...
    function toString()
    {
        return $this->wrap(parent::toString(), 'h' . $this->level, ' id="' . $this->id . '"');
    }
}
...
class ListContainerEx extends ElementEx
{
    ...
    function canContain(&$obj)
    {
        return !is_a($obj, 'ListContainerEx') || $this->tag == $obj->tag && $this->level == $obj->level;
    }
    ...
    function YTableEx($_value)
    {
        ...
        foreach ($_value as $val) {
            if (preg_match('/^(\s+)?(.+?)(\s+)?$/', $val, $matches)) {
                $align[] = $matches[1] != ''
                    ? (isset($matches[3]) && $matches[3] != '' ? ' align="center"' : ' align="right"')
                    : '';
                $value[] = $matches[2];
            } else {
                $align[] = '';
        ...
        for ($i = 0; $i < $count; $i++) {
            if ($colspan[$i]) {
                ...
                $str .=
                    '<td class="style_td"' . $align[$i] . $colspan[$i] . '>' . guiedit_make_link($value[$i]) . '</td>';
            }
        }
        $this->elements[] = $str;
    }
