...
// given a url path return the physical directory that it corresponds to
...
function GetCurrentFolder()
{
    ...
    if (!str_ends_with($sCurrentFolder, '/'))
        $sCurrentFolder .= '/';
    if (!str_starts_with($sCurrentFolder, '/'))
        $sCurrentFolder = '/' . $sCurrentFolder;

    // Ensure the folder path has no double-slashes
    while (str_contains($sCurrentFolder, '//')) {
...
//(function(){ let d=document.domain;while (true){try{ const A=window.parent.document.domain;break;}catch(e) {}d=d.replace(/.*?(?:\.|$)/,'');if (d.length===0) break;try{document.domain=d;}catch (e){break;}}})();
//EOF;
//
